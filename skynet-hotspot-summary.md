...
5. `POST /api/payment/verify` — called after charge succeeds:
   ...
   - Creates user in MikroTik User Manager via REST API
   - Assigns the correct profile (daily/monthly)
   - Returns `{ loginUrl: "http://192.168.100.1/login?username=SKY-7K3M&password=4829" }`
...
- After payment, browser auto-redirects to hotspot login URL — user types nothing
